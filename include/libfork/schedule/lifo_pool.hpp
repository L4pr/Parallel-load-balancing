 ...
 * @file lifo_pool.hpp
 *
 * @brief A work-stealing thread pool with LIFO scheduling.
 */
...
/**
 * @brief Workers event-loop for LIFO pool.
 */
inline void lifo_work(numa_topology::numa_node<impl::numa_context<busy_vars>> node) noexcept {
  ...
  while (!my_context->shared().stop.test(std::memory_order_acquire)) {
    ...
    if (submit_handle submissions = my_context->try_pop_all_lifo()) {
      resume(submissions);
      continue;
    }
  ...
  while (submit_handle submissions = my_context->try_pop_all_lifo()) {
    resume(submissions);
  }
}
...
/**
 * @brief A LIFO scheduler based on a traditional work-stealing thread pool.
 *
 ...
 *
 * __Note:__ The `lifo_pool` must not be destructed until all submitted tasks have reached a
...
class lifo_pool {
 ...
 public:
  /**
   * @brief Move construct a new lifo_pool object.
   */
  lifo_pool(lifo_pool &&other) noexcept = default;
  /**
   * @brief The lifo pool is not copyable.
   */
  lifo_pool(lifo_pool const &other) = delete;
  /**
   * @brief Move assign a lifo_pool object.
   */
  auto operator=(lifo_pool &&other) noexcept -> lifo_pool & = default;
  /**
   * @brief The lifo pool is not copy assignable.
   */
  auto operator=(lifo_pool const &other) -> lifo_pool & = delete;

  /**
   * @brief Construct a new lifo_pool object.
   *
   * @param n The number of worker threads to create, defaults to the number of hardware threads.
   * @param strategy The numa strategy for distributing workers.
   */
  explicit lifo_pool(std::size_t n = std::thread::hardware_concurrency(),
    ...
    [&]() noexcept {
      ...
      for (auto &&node : nodes) {
        m_threads.emplace_back(impl::lifo_work, std::move(node));
      }
...
static_assert(scheduler<lifo_pool>);
