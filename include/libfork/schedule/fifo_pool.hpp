 ...
 * @file fifo_pool.hpp
 *
 * @brief A work-stealing thread pool with FIFO scheduling.
 */
...
/**
 * @brief Workers event-loop for FIFO pool.
 */
inline void fifo_work(numa_topology::numa_node<impl::numa_context<busy_vars>> node) noexcept {
  ...
  while (!my_context->shared().stop.test(std::memory_order_acquire)) {
    ...
    if (submit_handle submissions = my_context->try_pop_all_fifo()) {
      resume(submissions);
      continue;
    }
  ...
  while (submit_handle submissions = my_context->try_pop_all_fifo()) {
    resume(submissions);
  }
}
...
/**
 * @brief A FIFO scheduler based on a traditional work-stealing thread pool.
 *
 ...
 *
 * __Note:__ The `fifo_pool` must not be destructed until all submitted tasks have reached a
...
class fifo_pool {
 ...
 public:
  /**
   * @brief Move construct a new fifo_pool object.
   */
  fifo_pool(fifo_pool &&other) noexcept = default;
  /**
   * @brief The fifo pool is not copyable.
   */
  fifo_pool(fifo_pool const &other) = delete;
  /**
   * @brief Move assign a fifo_pool object.
   */
  auto operator=(fifo_pool &&other) noexcept -> fifo_pool & = default;
  /**
   * @brief The fifo pool is not copy assignable.
   */
  auto operator=(fifo_pool const &other) -> fifo_pool & = delete;

  /**
   * @brief Construct a new fifo_pool object.
   *
   * @param n The number of worker threads to create, defaults to the number of hardware threads.
   * @param strategy The numa strategy for distributing workers.
   */
  explicit fifo_pool(std::size_t n = std::thread::hardware_concurrency(),
    ...
    [&]() noexcept {
      ...
      for (auto &&node : nodes) {
        m_threads.emplace_back(impl::fifo_work, std::move(node));
      }
...
static_assert(scheduler<fifo_pool>);
